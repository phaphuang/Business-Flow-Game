...
export default function ClassificationChallenge({ challenge, onAnswer, disabled }: Props) {
  ...
  const handleSubmit = () => {
    if (disabled) return;
    const userAnswer: Record<string, string[]> = { tool: [], technology: [], business: [] };
  ...
  const categories = [
    { key: 'tool', label: 'Digital Tools', bgColor: 'bg-blue-50', borderColor: 'border-blue-300', textColor: 'text-blue-700' },
    { key: 'technology', label: 'Digital Technologies', bgColor: 'bg-purple-50', borderColor: 'border-purple-300', textColor: 'text-purple-700' },
    { key: 'business', label: 'Business Concepts', bgColor: 'bg-green-50', borderColor: 'border-green-300', textColor: 'text-green-700' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {categories.map(({ key, label, bgColor, borderColor, textColor }) => (
          <Card
            key={key}
          ...
          >
            <h3 className={`font-bold text-sm mb-3 text-center ${textColor}`}>{label}</h3>
            <div className="space-y-2">
              {Object.entries(assignment)
                ...
                .map(([itemId]) => {
                  const item = getItemById(itemId);
                  return (
                    <div key={itemId} data-testid={`assigned-item-${itemId}`} className={`px-3 py-2 ${bgColor} border ${borderColor} rounded text-xs flex justify-between items-center gap-2`}>
